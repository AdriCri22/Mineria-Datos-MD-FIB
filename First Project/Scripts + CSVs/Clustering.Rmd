...
colnames(dd)[16] = "subd" # changing the name of y to subd(subscribed) because
# _y_ could also be the y-axis (personal preference)

# ggsave("poutcome.png", plot = p, width = 5, height = 5, device = "png")
```
...
### Euclidean distance if all variables are numerical (we don't use this)
...
### Move to Gower mixed distance to deal simultaneously with numerical and qualitative data (we should use this, I think)

#### Dissimilarity matrix do not include in actives the identifier variables nor the potential response variable

```{r}
# actives <- c(1, 6, 9, 11, 13, 14)
actives<-c(1:16)
...
par(mar=c(1,1,1,7))
dend %>% plot(horiz=FALSE, axes=FALSE)
abline(h = 2.1, lty = 2)
```
...
# png(filename = "cluster_split_color.png", width = 8.268, height = 11.693, units = "in", res = 800)
dend <- as.dendrogram(h1)
par(mar=c(1,1,1,7))
dend %>%
  set("labels_col", value = c("skyblue", "orange", "grey", "palegreen3"), k=4) %>%
  set("branches_k_color", value = c("skyblue", "orange", "grey", "palegreen3"), k = 4) %>%
...
```{r}
# png(filename = "cluster_split_color.png")
```
...
#comparing with other partitions #table(c1,c2)
```
...
```{r}
c1<-c2
plot(age, balance,col=c1,main="Clustering of credit data in 4 classes")
...
```{r}
# cdg <- aggregate(as.data.frame(dcon),list(c1),mean) cdg
# 
# plot(cdg[,1], cdg[,7])
# 
# #Tss <- h1$totss #Bss <- sum(rowSums(cdg^2)*as.numeric(table(c1)))
# 
# #Ib4 <- 100*Bss/Tss #Ib4
# 
# names(dd) #ratiFin boxplot(dd[,16]~c2, horizontal=TRUE)
# 
# #plazo boxplot(dd[,4]~c2, horizontal=TRUE)
# 
# #gastos boxplot(dd[,9]~c2, horizontal=TRUE)
# 
# pairs(dcon[,1:7], col=c2)
# 
# plot(RatiFin,Estalvi,col=c2,main="Clustering of credit data in 3 classes") legend("topright",levels(c2),pch=1,col=c(1:4), cex=0.6)
# 
# cdg <- aggregate(as.data.frame(dcon),list(c2),mean) cdg
# 
# plot(age, balance, col= c2) points(cdg[,4],cdg[,5],pch=16,col="yellow") text(cdg[,4],cdg[,5], labels=cdg[,1], pos=2, font=2, cex=1.2, col="yellow")
# 
# potencials<-c(3,4,6,7,10,11) pairs(dcon[,potencials],col=c2)
# 
# tt<-table(Tipo.trabajo,c2)
# 
# barplot(table(age, c2), beside=TRUE,col=c(1:length(levels(age))) ) legend("topright",levels(age),pch=1,cex=0.5, col=c(1:length(levels(age))))
# 
# barplot(table(Tipo.trabajo, c2), beside=FALSE,col=c(1:length(levels(Tipo.trabajo))) ) legend("topright",levels(Tipo.trabajo),pch=1,cex=0.5, col=c(1:length(levels(Tipo.trabajo))))
# 
...
data <- as.matrix(dd[, c(1, 6, 9, 11, 13, 14)])
heatmap(data)
```
...
All variables have already been transformed to factors
