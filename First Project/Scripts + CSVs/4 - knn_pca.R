...
#We remove any entry with missings in job or education
#dd3<-dd3[!(dd3$job=="unknown"),]
#dd3<-dd3[!(dd3$education=="unknown"),]
#dd3<-dd3[!(dd3$job=="unk"),]
#dd3<-dd3[!(dd3$education=="unk"),]

#we remove the variable contact because there's no way to fix the missings
dd3 <- subset(dd3, select = -(contact))

#summary(dd3$age)

for (k in 1:ncol(dd3)){
  if(is.character(dd3[,k])){
    dd3[,k]<-as.factor(dd3[,k])
  }
}

write.table(dd3, file="4 - PCA.csv", sep=";", dec = " ")
...
numeriques<-which(sapply(dd3,is.numeric))
numeriques
...
plot(Psi[,eje1],Psi[,eje2],type="n",xlim=c(-0.5,1), ylim=c(-0.5,1))
...
axis(side=2, pos= 0, labels = F, col="darkgray")
axis(side=4, pos= 0, labels = F, col="darkgray")
legend("bottomleft",levels(varcat),pch=1,col=c(1,2,3,4), cex=0.6)
