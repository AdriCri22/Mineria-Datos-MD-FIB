...
dcon <- data.frame(dd$age, dd$balance, dd$duration, dd$campaign, dd$pdays, dd$previous)
...
c1<-c2
plot(age, balance,col=c1,main="Clustering of credit data in 3 classes")
...
barplot(table(age, c2), beside=TRUE,col=c(1:length(levels(age))) )
legend("topright",levels(age),pch=1,cex=0.5, col=c(1:length(levels(age))))
